...
const Contact = () => {
  ...
  const handleEmailClick = () => {
    trackContactEvent('email_click', 'contact-page', {
      email: 'primeskyeastafrica@yahoo.com',
      section: 'contact-info'
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const emailSubject = encodeURIComponent(`Quote Request from ${formData.firstName} ${formData.lastName} - Primesky East Africa`);
      const emailBody = encodeURIComponent(`NEW QUOTE REQUEST - PRIMESKY EAST AFRICA
...
Company: +254 741 464497 | primeskyeastafrica@yahoo.com`);

      // Open mailto links for both email addresses
      const mailto1 = `mailto:primeskyeastafrica@yahoo.com?subject=${emailSubject}&body=${emailBody}`;
      const mailto2 = `mailto:estherzawadi887@gmail.com?subject=${emailSubject}&body=${emailBody}`;
      
      // Open first email
      window.open(mailto1, '_blank');
      
      // Wait 500ms then open second email
      setTimeout(() => {
      ...
      setStatusMessage(
        `Thank you ${formData.firstName}! Email clients have been opened for both primeskyeastafrica@yahoo.com and estherzawadi887@gmail.com. ` +
        `Please send the emails from your email client. We'll contact you at ${formData.phone} within 24 hours.`
      );
      ...
      // Try FormSubmit in background as backup
      const emailPromises = [];
      ...
      formSubmitData.append('Company_Email', 'primeskyeastafrica@yahoo.com');

      // Send to both emails with slight delay to avoid rate limiting
      emailPromises.push(
        fetch('https://formsubmit.co/primeskyeastafrica@yahoo.com', {
          method: 'POST',
          body: formSubmitData
        })
      );
      ...
      formSubmitData2.append('Company_Email', 'primeskyeastafrica@yahoo.com');
...
Company: +254 741 464497 | primeskyeastafrica@yahoo.com`;
        ...
        localStorage.setItem('lastQuoteRequest', JSON.stringify({
          ...
          recipients: ['primeskyeastafrica@yahoo.com', 'estherzawadi887@gmail.com']
        }));
